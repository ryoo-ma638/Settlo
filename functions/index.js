  ...
  async (request) => {
    try {
      ...
      const prompt = `
        ...
        【消費税の扱い（重要）】
        - "totalAmount" は必ず「レシートに印字された、実際に支払った最終合計（税込）」にしてください。ここが最も正確な正データです。
        - 各商品の "price" は「その商品が最終合計に寄与する金額（税込・数量ぶん込み）」にしてください。
          レシートが税抜表示の場合は、その商品に対応する税率（軽減8% / 標準10%）を掛けて税込に換算してください。
          判別できない場合は、食品・飲料は8%、それ以外は10%として税込換算してください。
        - "taxIncluded" は、商品の price 欄がもともと税込表示だったなら true、税抜表示だったなら false。
        - 各商品ごとに適用されている税率が分かれば "taxRate" に 8 か 10 を入れてください（不明なら null）。
        - 注意：各商品を個別に税込にして合計すると、店側が小計に一括で課税している関係で totalAmount と数円ずれることがあります。
          その場合でも "totalAmount" はレシートの印字どおりの最終合計を優先してください（ずれはアプリ側で調整します）。
        ...
        見つかれば "T" を含む形の文字列で入れてください。見つからなければ null にしてください。
        {
          ...
          "items": [
            { "name": "商品名", "price": その商品の税込合計金額(数値), "quantity": 個数(数値、不明な場合は1), "taxRate": 8または10または null }
          ]
        }
      `;
