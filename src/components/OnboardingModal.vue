...
const slides = [
  ...
  },
  {
    ...
    tap: '右上のベルにお知らせ、隣のロボットが次にやることを案内',
  },
  {
    title: '上のアイコンの意味',
    text: 'ヘッダーは中央がロゴ（押すとホーム）。左から自分のアイコン（マイページ）、時計は「承認待ち」、吹き出しは「チャット」（未読数バッジ）。右はベルが「お知らせ」、右端のロボットが「お支払いアシスタント」で、いま承認・支払い・催促する相手を金額つきで教えてくれます。',
    tap: '使い方はマイページの「アプリの使い方」から、いつでも見られます',
  },
  {
    title: '支払いのやり取り',
    text: '相手に「支払う」／自分が「受け取る」／まだの人に「催促する」。相手が支払ったら「承認」で完了、違えば「拒否」。事情を聞きたいときは「返信」でチャットができます。削除や退出には理由を添えられるので、もめにくくなっています。',
    tap: '解決した支払いのチャットは自動で片付きます',
  },
];
