<template>
  ...
  const props = defineProps({
    isOpen: Boolean,
    eventCode: { type: String, default: 'EO2Q2Z' } // デフォルトの招待コード
  });
  ...
  const invitedSet = ref(new Set()); // 招待した人のIDを保存
  
  const invite = (id) => {
    invitedSet.value.add(id);
    // 実際はここでバックエンドに招待リクエストを送る
  };
  ...
  // 🌟 ダミーデータ（Settloを利用している全ての人。検索で完全一致したら出てくる）
  const globalData = [
