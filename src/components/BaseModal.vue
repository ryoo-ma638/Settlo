<template>
  ...
  <style scoped>
  /* オーバーレイ（背景の暗がり） */
  .modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); display: flex; justify-content: center; align-items: center; z-index: 9999; backdrop-filter: blur(4px); }
  
  /* モーダル本体（iOS風のクリーンなデザイン） */
  .modal-container { background: white; width: 90%; max-width: 320px; border-radius: 24px; padding: 24px; box-shadow: 0 20px 60px rgba(0,0,0,0.2); text-align: center; overflow: hidden; }
  ...
  /* テキスト */
  .modal-title { font-size: 18px; font-weight: 800; color: #1e293b; margin: 0 0 8px 0; }
  .modal-message { font-size: 14px; color: #64748b; margin: 0 0 24px 0; line-height: 1.5; }
