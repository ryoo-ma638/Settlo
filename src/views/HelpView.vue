<template>
  <div class="help">
    ...
    <main class="help__body">
      ...
      <div class="map">
        <div class="map__row"><span class="map__from">下のナビ</span><span class="map__to">ホーム ／ イベント ／ ＋（イベントを作成・お支払いを追加・フレンドと割り勘） ／ 支払い ／ フレンド</span></div>
        <div class="map__row"><span class="map__from">ヘッダー</span><span class="map__to">ロゴ→ホーム ／ 顔→マイページ ／ 時計→承認待ち ／ 吹き出し→相談 ／ ベル→お知らせ ／ ロボット→アシスタント</span></div>
        ...
        <div class="map__row"><span class="map__from">マイページ</span><span class="map__to">→ プロフィールを変更 ／ 通知設定 ／ フレンド ／ お支払い履歴 ／ 承認待ち ／ 相談 ／ 元に戻す ／ このヘルプ</span></div>
...
const steps = [
  { img: 'g-home', title: '① ホームで貸し借りがひと目', body: '大きい数字は<b>いま受け取る額</b>と<b>いま払う額</b>で、すぐ下に残りの件数が出ます。灰色の枠は相手の返事を待っている分で、上の金額には入っていません。進行中のイベントもここから開けます。' },
  ...
  { img: 'g-addpay', title: '③ お支払いを追加する', body: 'イベント内の<b>「＋ 支払いを追加」</b>から立て替えを記録します。', points: [
    '<b>レシートを撮ると、AIが店名・金額・消費税まで自動入力</b>（手入力もOK）。1回に5枚までまとめて読み取れます',
    '<b>立替えた人</b>を選ぶ（自分／デモ太郎 など）',
    '<b>割り勘の方法</b>：全員で均等／金額を指定／商品ごとに指定',
    '最後に<b>「この内容で追加する」</b>で登録',
  ] },
  { img: 'g-settle', title: '④ まとめて精算する', body: '精算のしかたは2つあります。<b>「支払い」→「まとめて」</b>タブは、相手ごとに全イベントの貸し借りを相殺します。イベント画面の<b>「まとめて精算」</b>は、参加者全員の貸し借りを一度にまとめ、送金の回数がいちばん少なくなる組み合わせを出します。' },
  ...
  { img: 'g-split', title: '⑧ イベントを作らずに1件だけ割り勘', body: '下の<b>「＋」→「フレンドと割り勘」</b>から相手を選び、品名・金額・立て替えた人・割り方を入れます。記録する前に<b>「デモ花子さんが自分へ ¥2,400 を支払う記録になります。」</b>と結果が出ます。いつものメンバーで軽く出し合うときに、旅行や飲み会の箱を作らずに使えます。' },
];
...
const screenGroups = [
  {
    ...
    screens: [
      ...
      { name: 'ホーム', route: '', icon: 'home',
        ...
        items: [
          { b: '現在の精算状況（左右で切替）', act: 'タップ → 支払い（受け取り／未払いのタブ）' },
          { b: '大きい数字の下の件数', act: 'あと何件やり取りが残っているか' },
          { b: '灰色の枠（確認待ち）', act: '相手の返事を待っている分。上の金額には入りません' },
      ...
      { name: '支払い・精算', route: '', icon: 'wallet',
        ...
        items: [
          ...
          { b: 'イベントでまとめて精算中', act: 'イベント側でまとめた分。押すとそのイベントの精算画面へ' },
      ...
      { name: 'イベント一覧', route: '', icon: 'calendar',
        ...
        items: [
          ...
          { b: 'コードで参加', act: '→ 招待コードの入力' },
          { b: '進行中／終了済み', act: '一覧の切替' },
          { b: 'イベントカード', act: '→ そのイベント詳細（支払い追加から来たときは追加画面つき）' },
          { b: '非表示にしたイベント', act: '削除したイベントを一覧へ戻す（あるときだけ出ます）' },
        ] },
      { name: 'イベント詳細', route: '', icon: 'calCheck',
        ...
        items: [
          ...
          { b: 'まとめて精算', act: '参加者全員の貸し借りをまとめ、送金の回数を減らした結果を出す' },
          { b: '精算サマリーの送金カード', act: 'まとめて支払う／受け取る・催促へ' },
          { b: '立て替え履歴のカード', act: 'その取引の詳細（編集・削除・未精算に戻す）' },
          { b: 'イベントから退出', act: '未精算が残っていても抜ける（お金の記録は支払い画面に残る）' },
          { b: 'イベントを終了する', act: '全員の精算が済んだら締める。ほかの人には「あなたも終了しますか？」が届き、全員が終えたときに終了' },
          { b: 'イベントを再開する', act: '終了したあとに支払いの追加を再開する' },
          { b: 'イベントを削除する', act: '自分の画面から非表示にする（相手の画面には残る。「非表示にしたイベント」から戻せる）' },
        ] },
      { name: 'フレンド', route: '', icon: 'people',
        purpose: '友達の一覧・追加・申請の承認。相手ごとの貸し借りも反映されます。＋の「フレンドと割り勘」から来たときは、相手を選ぶ画面になります。',
        items: [
      ...
      { name: 'フレンド詳細', route: '', icon: 'people',
        purpose: 'この人との貸し借りと取引履歴。イベントを作らずに1件だけ記録することもできます。',
        items: [
          { b: '支払う分・受け取る分を選ぶ', act: '→ まとめて精算する内容を選ぶ' },
          { b: 'この人と割り勘を記録する', act: 'イベント無しで立て替えを1件だけ登録' },
          { b: 'この人との会話を見る', act: '→ この相手が関わる相談の一覧' },
          { b: '取引履歴のカード', act: '→ その支払いの詳細' },
          { b: 'ゴミ箱アイコン（右上）', act: 'フレンドから外す' },
        ] },
  ...
  },
  {
    ...
    screens: [
      ...
      { name: 'スレッド（会話）', route: '', icon: 'bubble',
        purpose: '1件についての会話。承認や支払い画面への近道もあります。',
        items: [
  ...
  },
  {
    title: '安心・その他',
    screens: [
      { name: '元に戻す', route: '', icon: 'trash',
        purpose: '削除した立て替えの復元、精算済みを未精算へ戻す依頼、片付けたお知らせの取り消しを行います。',
        items: [
          { b: 'タブ（復元できる取引／確認中）', act: '自分が操作できる分と、相手の確認待ちで切替' },
          { b: '元に戻す／未精算に戻す', act: '削除前の状態へ復元（相手に確認が届きます）' },
          { b: '確認中', act: '相手の確認待ち。同じ操作は繰り返せません' },
          { b: '片付けたお知らせ', act: '「過去のお知らせ」からここへ移した分。戻すとお知らせに返ります' },
        ],
        tip: '共有する記録は手動では消せません。ここへ移したものは7日で自動的に整理されます。相手の確認待ちのものは、返事があるまで消えません。相手からも見えるので、勝手に消されても気づけます。' },
      { name: '通知設定', route: '', icon: 'bell',
        purpose: '端末への通知を受け取るかどうかを決めます。',
        items: [
          { b: '通知を受け取る', act: 'ブラウザの許可を求めてから有効になります' },
          { b: '種類ごとの切替', act: '催促・承認・相談など、受け取る通知を選べます' },
        ],
        tip: '通知を切っても、アプリを開けばお知らせのベルに届いています。' },
