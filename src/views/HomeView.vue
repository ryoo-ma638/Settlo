...
/* 🌟 修正： 85vw を 85% に変更（画面全体ではなく、中央カラムの85%にする） */
.status-card { flex: 0 0 85%; background-color: white; border-radius: 20px; padding: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); scroll-snap-align: center; min-height: 140px; display: flex; align-items: center; box-sizing: border-box; }
...
/* 🌟 追加：PC版（1024px以上）専用のスタイル調整 */
@media (min-width: 1024px) {
  .home-container {
    padding-bottom: 40px; /* スマホ用フッターの余白を消す */
  }
  .add-button {
    /* 🌟 修正：PC版では左メニューにボタンがあるので、右下の丸い＋ボタンは完全に隠す！ */
    display: none; 
  }
}
</style>